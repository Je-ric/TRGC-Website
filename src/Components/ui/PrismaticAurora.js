export function PrismaticAurora({ children, className = "" }) {
    return (
        <div className={`relative w-full bg-black ${className}`}>
            {/* Prismatic Aurora Burst - Top and Bottom */}
            <div
                className="absolute inset-0 z-0"
                style={{
                    background: `
            /* Top aurora */
            radial-gradient(ellipse 120% 80% at 70% 20%, rgba(255, 20, 147, 0.15), transparent 50%),
            radial-gradient(ellipse 100% 60% at 30% 10%, rgba(0, 255, 255, 0.12), transparent 60%),
            radial-gradient(ellipse 90% 70% at 50% 0%, rgba(138, 43, 226, 0.18), transparent 65%),
            radial-gradient(ellipse 110% 50% at 80% 30%, rgba(255, 215, 0, 0.08), transparent 40%),
            /* Bottom aurora */
            radial-gradient(ellipse 120% 80% at 30% 80%, rgba(255, 20, 147, 0.15), transparent 50%),
            radial-gradient(ellipse 100% 60% at 70% 90%, rgba(0, 255, 255, 0.12), transparent 60%),
            radial-gradient(ellipse 90% 70% at 50% 100%, rgba(138, 43, 226, 0.18), transparent 65%),
            radial-gradient(ellipse 110% 50% at 20% 70%, rgba(255, 215, 0, 0.08), transparent 40%),
            #000000
            `,
                }}
            />
            {/* Content */}
            <div className="relative z-10">{children}</div>
        </div>
    );
}

export default PrismaticAurora;
